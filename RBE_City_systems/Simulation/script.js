...
let vehicles = []; // Array to hold our transportation vehicles
...
// Function to create the Transportation System
function createTransportationSystem() {
    const vehicleCount = 10;
    const vehicleColor = 0xffe082; // Light yellow
    const vehicleRadius = 0.5;
    const pathRadius = 15;

    // Create vehicles
    for (let i = 0; i < vehicleCount; i++) {
        const vehicleGeometry = new THREE.SphereGeometry(vehicleRadius, 16, 16);
        const vehicleMaterial = new THREE.MeshPhongMaterial({ color: vehicleColor });
        const vehicle = new THREE.Mesh(vehicleGeometry, vehicleMaterial);
        
        // Store its current position on the path and speed
        vehicle.userData.angle = (i / vehicleCount) * Math.PI * 2;
        vehicle.userData.speed = 0.01 + Math.random() * 0.01;
...
// Function to create the Labs & Workshops system
function createLabsWorkshopsSystem() {
    // Place the labs system in the second sector (i=1)
    const sectorRadius = 10;
    const angle = 1 * Math.PI / 3;
    const x = Math.cos(angle) * (sectorRadius * 2);
    const z = Math.sin(angle) * (sectorRadius * 2);
    
    // Create a group to hold the labs system objects
    const labsGroup = new THREE.Group();
    labsGroup.position.set(x, 0, z);

    // Central core/tower of the lab complex
    const coreGeometry = new THREE.CylinderGeometry(5, 5, 10, 32);
    const coreMaterial = new THREE.MeshPhongMaterial({ color: 0xcccccc, transparent: true, opacity: 0.8 });
    const core = new THREE.Mesh(coreGeometry, coreMaterial);
    core.position.y = 5;
    labsGroup.add(core);

    // Interconnected research pods
    const podGeometry = new THREE.BoxGeometry(4, 3, 4);
    const podMaterial = new THREE.MeshPhongMaterial({ color: 0x999999 });
    ...
    podPositions.forEach(pos => {
        const pod = new THREE.Mesh(podGeometry, podMaterial);
        pod.position.set(pos.x, 1.5, pos.z);
        labsGroup.add(pod);
    });
...
function animate() {
    ...
    // Animate the transportation vehicles
    vehicles.forEach(vehicle => {
        vehicle.userData.angle += vehicle.userData.speed;
        vehicle.position.x = Math.cos(vehicle.userData.angle) * 15;
        vehicle.position.z = Math.sin(vehicle.userData.angle) * 15;
    });
